...
~~**无在飞分配且无 ready = 有意空闲**~~ **作废**(`4e2e090`;留档 ⛔ 删——夜二停手 9h 的条文根)。
**现行口径见起窗打印的 `DISPATCH_BRIEF`**:三桶是事实面,**可自写非空 ⇒ 写单就是当下动作**;**「可立即发车 = 0」⛔ 等于空闲**。
...
- Claude 派工席：70–75% 是**起交接**区间 ⛔ 上限——起交接后前任继续把在手的活盯完，ctx 超出属正常；写数字前当轮跑 laixin-lane ctx <session前8位>。
- 🔴 **「起交接」＝当轮跑 `laixin-lane dispatch --handover`，⛔ 等于写完交接包**——它把旧窗改名 `dispatch-drain-*`、起继任、移交派工权。**前任自己跑**（禁令面只禁 `git push`）；⛔ 请创始人开窗 ⛔ 转他席代跑。
- 🔴 **跑完当轮核两条，任一不成立＝交接没开始**：`ListAgents` 见继任名 · `whoholds` 派工权已转。**没开始 ⇒ 职责一秒未转移，新活仍归前任 ⛔ 挂「继任」名下**（实撞 2026-08-30 空转 2h20m：两端状态各自看都正常，只能靠「整体在不在动」发现）。
- Codex 开发/验收席自带翻页，不因占比高人工换班。
- Kimi 看自身 TUI context，不要用 laixin-lane ctx 读别人的数。
- 交接期两窗同步跑，前任逐步退去：**新活一律归继任 ⛔ 前任再接**，前任只把起交接那刻已在手的收尾；**关窗判据＝在手全盯完 加 优化轮完成**（权威位＝协作流程 交班卡节）。
- 交接包写待验、ready、挂起、第一动作与权威路径；**在途件是告知件 ⛔ 移交件**；继任未接稳前保持注册表“待接”。
